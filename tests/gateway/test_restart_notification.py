...
async def test_send_home_channel_startup_notification_default_flag_true(
    ...
    """Explicit operator/back-channel opt-in allows notifications to fire."""
    monkeypatch.setattr(gateway_run, "_hermes_home", tmp_path)

    runner, adapter = make_restart_runner()
    # The shared restart test runner opts in so delivery-path tests keep
    # exercising successful sends while production defaults stay silent.
...
@pytest.mark.asyncio
async def test_shutdown_notifications_suppressed_by_display_policy(monkeypatch):
    monkeypatch.delenv("HERMES_GATEWAY_LIFECYCLE_NOTIFICATIONS_ENABLED", raising=False)
    monkeypatch.setattr(
        gateway_run,
        "_load_gateway_config",
        lambda: {"display": {"lifecycle_notifications_enabled": False}},
    )
